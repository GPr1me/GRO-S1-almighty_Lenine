...
avancer(170, 24, 0, 0.9);

    avancer(0, 12, 0.9, 0.8);
    ...
    tourner(0.7, -3.0, 85); //Rayon final après 180 deg

    //avancer(0, 20, 0.7, 0.9);
    
    tourner(0.7,50,30); //aller au 90 deg
    //avancer(4, 24, 0.9, 0.9); //avance jusqu'au 90 deg

    //avancer(0, 12, 0.9, 0.6);

    avancer(10, 6, 0.7, 0.7);
    
    tourner(0.7, -3.0, 70); //tournant après 90 deg

    avancer(47, 10, 0.7, 0.9);

    avancer(0, 10, 0.9, 0.7);

    tourner(0.7 ,3.0, 36); //grand arc

    avancer(22, 10, 0.7, 0.7);

    tourner(0.7, 3.0, 10);

    avancer(57, 20, 0.7, 0.9);

    // tourner(0.7,8,4); //positionnement final

    // avancer(0, 12, 0.6, 0.9);
    
    // avancer(10, 12, 0.9, 0.9); //ligne après le 90 deg

    // avancer(0, 6, 0.9, 0.7);
    
    // tourner(0.7, 8, 47); //avant dernier angle

    // avancer(0, 12, 0.7, 0.9);

    // avancer(3, 12, 0.9, 0.9); //avant dernière ligne

    // avancer(0, 12, 0.9, 0.7);

    // tourner(0.7, 8, 8); //dernier tournant

    // avancer(0, 24, 0.7, 0.9);

    // avancer(42, 12, 0.9, 0.9); //DERNIERE STRETCH!
    ...
    avancer(-66, 20, 0, -0.7); // Premiere ligne retour

    avancer(0, 10, -0.7, -0.4); // Deceleration

    tourner(-0.4, 12.0, 12); // Premier tournant retour
     ...
     tourner(-0.4, 12.0, 38); // tournant pour ligne 3

     avancer(0, 10, -0.4, -0.7); // accel ligne 3

     avancer(-21, 10, -0.7, -0.7); // ligne 3
     ...
     avancer(0, 10, -0.4, -0.7); // accel ligne 4
      ...
      avancer(-7.0, 5, -0.7, -0.7); // ligne 4

      avancer(0, 10, -0.7, -0.4); // decel ligne 4

      tourner(-0.4, 3.0, 62); // tournant pour ligne 5

      tourner(-0.4, -3.0, 95); // tournant pour ligne 6

      avancer(-16, 10, -0.4, -0.4);

      tourner(-0.4, 4.0, 87); //tournant pour U turn 1/2

      tourner(-0.4, 18.0, 90); //tournant pour U turn 1/2

      tourner(-0.4, -7.0, 102); //tournant vers ligne finale
